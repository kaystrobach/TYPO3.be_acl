	...
	function select(&$PA, $fobj) {
		...
		switch($PA['row']['type'][0]) {
			...
			case '0':
				...
				if(!$GLOBALS['BE_USER']->isAdmin()) $items = BackendUtility::blindUserNames($items, $BE_USER->userGroupsUID, 1);
			...
			case '1':
				...
				if(!$GLOBALS['BE_USER']->isAdmin()) $items = BackendUtility::blindGroupNames($items, $BE_USER->userGroupsUID, 1);
